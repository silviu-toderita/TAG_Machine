...
int16_t timezone_offset; //Offset from UTC in hours
...
void NTP_Clock::send_NTP_packet(){ 
    ...
    UDP.beginPacket(server_address_IP, 123);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
bool NTP_Clock::handle(){ 
    uint32_t interval;

    if(time_at_last_response == 0){ //If we've not received any time yet, the interval is 2s. Otherwise the interval is normal. 
        interval = 2000;
    }else{
    ...
    if(millis() - last_request_millis > interval){ //If it's been more than the interval since the last NTP Request, send another.
        last_request_millis = millis();
        send_NTP_packet();
    }

    if(UDP.parsePacket() != 0){ //If there is any data in the UDP buffer, process it
        UDP.read(NTP_buffer, NTP_packet_size); //Read the buffer
...
bool NTP_Clock::begin(){
    ...
    if(timezone == -1)  timezone = get_timezone();
    if(timezone == -1)  timezone = get_timezone();
    if(timezone == -1)  timezone = -420;

    return begin(3000, timezone);
}
...
uint8_t NTP_Clock::get_leap_years(){ 
    ...
    return (get_UNIX_time() - 1483228800) / 126230400;
}
...
uint16_t NTP_Clock::get_year(){
    ...
    return ((get_UNIX_time() - 1546300800 - (get_leap_years() * 86400)) / 31536000) + 2019;
}
